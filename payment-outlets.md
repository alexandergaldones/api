...
Users can receive their fiat currency payouts via selected Payment Outlets. Coins.ph supports multiple Payment Outlets, and we are regularly adding additional payment outlets.

## Payment Outlet Types

Payment Outlets are grouped into several categories. These categories are described in their respective sections.

### Bank

Users can opt to receive their payout through a bank account. These establishments require the following details for a payout to be successful:
...
Technically, Payment Outlets of this category do not have a minimum and a maximum amount limit per transaction. However, usual bank policies still apply for higher amounts, and the recipient might need to ask the bank for reservation beforehand. It is the responsibility of the user/recipient to ask the bank for reservation.
...
Users can opt to receive their payout by picking it up on an establishment that allows cash pickup. These establishments require the following details for a payout to be successful:
...
Additionally, these establishments usually require the recipient to present at least one valid ID (preferrably government issued). The information provided by the user should exactly match the details in the recipient's ID.
...
Certain establishments of this category only accepts payouts within a certain range. These values are as of Oct. 21, 2014:
...
Users can opt to receive their payout through a mobile number that supports mobile money (such as GCash and Smart Money). These payout options only require the recipient's phone number. The user can opt to send the payout to a different user.
...
Certain services of this category only accepts payouts within a certain range. These values are as of Oct. 21, 2014:
...
Users can opt to receive their payout through delivery. These services require the user to provide the following details:
...
Additionally, the recipient might be required to present at least one valid ID (preferrably government issued) upon delivery. The details provided in the sell order should match the exact details in the recipient's ID.
...
Services of this category only accepts payouts within a certain range. These values are as of Oct. 21, 2014:
